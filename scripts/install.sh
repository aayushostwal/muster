...
INTERACTIVE_TERMINAL_ENABLED="${MUSTER_INTERACTIVE_TERMINAL_ENABLED:-$(installed_value MUSTER_INTERACTIVE_TERMINAL_ENABLED)}"
INTERACTIVE_TERMINAL_ENABLED="${INTERACTIVE_TERMINAL_ENABLED:-false}"
DEFAULT_TASK_RUNTIME_MODE="${MUSTER_DEFAULT_TASK_RUNTIME_MODE:-$(installed_value MUSTER_DEFAULT_TASK_RUNTIME_MODE)}"
DEFAULT_TASK_RUNTIME_MODE="${DEFAULT_TASK_RUNTIME_MODE:-structured}"
...
validate_settings() {
  ...
  case "$INTERACTIVE_TERMINAL_ENABLED" in
    true|false) : ;;
    *) die "MUSTER_INTERACTIVE_TERMINAL_ENABLED must be true or false" ;;
  esac
  case "$DEFAULT_TASK_RUNTIME_MODE" in
    structured|interactive) : ;;
    *) die "MUSTER_DEFAULT_TASK_RUNTIME_MODE must be structured or interactive" ;;
  esac
  if [ "$DEFAULT_TASK_RUNTIME_MODE" = "interactive" ] && [ "$INTERACTIVE_TERMINAL_ENABLED" != "true" ]; then
    die "MUSTER_DEFAULT_TASK_RUNTIME_MODE=interactive requires MUSTER_INTERACTIVE_TERMINAL_ENABLED=true"
  fi
...
MUSTER_INTERACTIVE_TERMINAL_ENABLED=${INTERACTIVE_TERMINAL_ENABLED}
MUSTER_DEFAULT_TASK_RUNTIME_MODE=${DEFAULT_TASK_RUNTIME_MODE}
MUSTER_TERMINAL_ALLOWED_ORIGINS=http://localhost:${FRONTEND_PORT},http://127.0.0.1:${FRONTEND_PORT}
EOF
)
...
render_service_template() {
  ...
  RENDER_TERMINAL_ENABLED="$INTERACTIVE_TERMINAL_ENABLED" \
  RENDER_DEFAULT_RUNTIME_MODE="$DEFAULT_TASK_RUNTIME_MODE" \
  RENDER_TERMINAL_ORIGINS="http://localhost:${FRONTEND_PORT},http://127.0.0.1:${FRONTEND_PORT}" \
...
values = {
    ...
    "TERMINAL_ENABLED": os.environ["RENDER_TERMINAL_ENABLED"],
    "DEFAULT_RUNTIME_MODE": os.environ["RENDER_DEFAULT_RUNTIME_MODE"],
    "TERMINAL_ORIGINS": os.environ["RENDER_TERMINAL_ORIGINS"],
}

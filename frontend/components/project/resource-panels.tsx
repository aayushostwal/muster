...
export function ToolPanel({ projectId }: { projectId: string }) {
  ...
  return <Panel title="Tool permissions" description="Allow or deny runtime tools for this project across all granted directories." action={<Button variant="primary" size="sm" onClick={() => setOpen(true)}><Plus className="h-4 w-4" /> Add permission</Button>}>
    {query.isPending ? <ListLoading /> : query.isError ? <ErrorState message={query.error.message} retry={() => query.refetch()} /> : query.data.items.length === 0 ? <EmptyState title="No explicit tool permissions" description="Agents will ask interactively whenever the runtime requires permission." /> : <div className="space-y-2">{query.data.items.map((item) => { const configured = Boolean(item.config.backend && item.config.decision); const runtime = item.config.backend === "all" ? "Claude + Codex" : item.config.backend ? backendLabel(item.config.backend) : "Legacy rule"; return <ResourceRow key={item.id} icon={<Code2 className={cn("h-4 w-4", item.config.decision === "allow" ? "text-signal-400" : item.config.decision === "deny" ? "text-red-400" : "text-slate-500")} />} title={item.name} meta={configured ? `${item.config.decision} · ${runtime}` : "Inactive legacy configuration"} detail={<div className="space-y-2 font-mono text-[0.68rem] leading-5 text-slate-500">{item.config.claude_pattern && <p>Claude: {item.config.claude_pattern}</p>}{item.config.codex_prefix?.length ? <p>Codex: {item.config.codex_prefix.join(" ")} *</p> : null}{!configured && <p>Recreate this rule to activate runtime enforcement.</p>}</div>} actions={<DeleteButton label={item.name} loading={remove.isPending} onDelete={() => remove.mutate(item.id)} />} />; })}</div>}

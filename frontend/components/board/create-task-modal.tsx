...
export function CreateTaskModal({ projectId, open, onClose }: { projectId: string; open: boolean; onClose: () => void }) {
  ...
  const create = useMutation({
    mutationFn: () => api.createTask(projectId, { title: title.trim(), initial_prompt: prompt.trim(), backend: backend === "inherit" ? undefined : backend, model: models[0], fallback_models: models.slice(1), thinking_level: thinking, agent_id: agentId || undefined }),

...
const stageOrder: TaskStatus[] = ["waiting_on_you", "running", "queued", "failed", "done", "cancelled"];
const scopeOptions: Array<{ value: BoardScope; label: string; statuses: TaskStatus[] }> = [
  { value: "all", label: "All work", statuses: stageOrder },
  { value: "active", label: "Active", statuses: ["running", "queued"] },
  { value: "attention", label: "Needs attention", statuses: ["waiting_on_you", "failed"] },
...
export function TaskBoard({ projectId }: { projectId: string }) {
  ...
  const visibleStages = stageOrder.filter((status) => {
    ...
    return inScope && (showEmpty || visibleTasks.some((task) => task.status === status));
  });
  ...
  return (
    <div className="mx-auto max-w-[128rem] px-3 py-4 sm:px-5 md:px-7 md:py-6">
      ...
      {metrics.attention > 0 && <AttentionRail tasks={allTasks.filter((task) => task.status === "waiting_on_you" || task.status === "failed")} onShowAll={() => setScope("attention")} />}
...
function TaskTag({ value }: { value: string }) {
  const style = value === "Need Attention" ? "border-amber-400/25 bg-amber-400/[0.09] text-amber-300" : TASK_TAG_COLORS[value] || "border-white/[0.08] bg-white/[0.035] text-slate-400";
...
function getMetrics(tasks: Task[]) {
  ...
  const attention = tasks.filter((task) => task.status === "waiting_on_you" || task.status === "failed").length;
